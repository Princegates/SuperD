...
supabase/
  migrations/
    ...
    0075_fix_vendor_subscription_currency_ambiguity.sql   fixes charge_vendor_subscription_precheck() failing every "Pay via Mobile Money" attempt with "column reference \"currency\" is ambiguous" - its own output column shared a name with a variable PL/pgSQL implicitly declared for it
    ...
    0085_flat_twelve_percent_commission.sql     switches riders from the tiered daily fee to commission only: empties driver_daily_fee_tiers (so driver_daily_fee_amount() resolves to 0) and sets app_settings.commission_percentage to 12 with no flat fee, the rate a super admin edits in Console > Settings. No new machinery - the existing sheet already charges driver_total_amount_due() (fee balance + commission), which with no tiers is commission alone, and driver_has_overdue_commission() becomes the only block on new deliveries
