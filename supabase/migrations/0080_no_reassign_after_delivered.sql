-- SuperD: a dispatcher/super admin could still change a delivery's
-- assigned driver from the Console after it was already marked
-- 'delivered' - the driver dropdown on DeliveryDetailAdminScreen only
-- disabled itself for a cancelled delivery, and enforce_delivery_update()
-- never checked delivery status before accepting an assigned_driver_id
-- change from a dispatcher-or-above caller (only a plain driver caller
-- was blocked from touching that column post-delivery, and even then
-- only indirectly via the broader "delivered can't be undone" status
-- guard in 0063_no_undo_after_delivered.sql - that guard is about
-- `status`, not `assigned_driver_id`, and only applies inside the
-- `not is_dispatcher_or_above()` branch).
--
-- That's a real problem, not just a client nicety: a delivery's driver
-- assignment is what commission/payment settlement (see
-- 0067_daily_commission_settlement.sql) and driver earnings are tied to
-- once it's done - changing it after the fact would rewrite who a
-- completed job actually belongs to. None of the auto-assignment or
-- driver-initiated code paths ever touch assigned_driver_id on an
-- already-delivered row (they're all scoped to 'pending'/'assigned'/
-- 'picked_up'/'in_transit'), so this can only ever be someone
-- deliberately overriding the completed record - never blocks a
-- legitimate flow.
--
-- Fixed on both sides, same pattern as 0063: the client also disables
-- the "Assigned driver" dropdown for a delivered delivery (previously
-- only cancelled), and this migration adds the server-side backstop,
-- unconditionally (unlike 0063's guard, this one is NOT scoped inside
-- the `not is_dispatcher_or_above()` branch - a dispatcher/admin from
-- the Console is exactly who this needs to stop).
